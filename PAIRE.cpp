...
int playMove(Position * pos_next, Position * pos_current, int computer_play, int indexCells) {
    if (pos_current->state){
        ...
        if (cpt == 0){
            if (computer_play) {
                for (int i = 0; i < SIZE12; i++) {
                    pos_next->seeds_computer += pos_next->cells24[2 * i + 1];
                    pos_next->cells24[2 * i + 1]=0;
                }
            }
            else {
                for (int i = 0; i < SIZE12; i++) {
                    pos_next->seeds_player += pos_next->cells24[2 * i];
        ...
        if (cpt == 0){
            if (computer_play) {
                for (int i = 0; i < SIZE6; i++) {
                    pos_next->seeds_computer += pos_next->cells12[2 * i + 1];
                    pos_next->cells12[2 * i + 1]=0;
                }
            }
            else {
                for (int i = 0; i < SIZE6; i++) {
                    pos_next->seeds_player += pos_next->cells12[2 * i];
...
void HUMANplay(Position * pos_next, Position * pos_current) {
    ...
    while (!move) {
        ...
        std::cin >> playedCell;
        move = validMove(pos_current, pos_current->computer_play, (playedCell / 2));
    }
    playMove(pos_next, pos_current, pos_current->computer_play, (playedCell / 2));
    showGame(pos_next);
}
...
void IAplay(Position * pos_next, Position * pos_current) {
    if (pos_current->state){
        ...
        std::cout << "IA PLAY #################### " << 2 * minMaxMove + 2 << " ####################" << std::endl;
